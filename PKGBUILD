...
if [ -z "$_driver_version" ] || [ "$_driver_version" = "latest" ] || [ -z "$_driver_branch" ] && [ ! -e options ]; then
  ...
  if [[ -z $CONDITION ]]; then
    ...
    # This (condition 1) will be treated as the latest Vulkan developer driver.
      ...
      echo '_driver_version=595.44.07' > options
      echo '_md5sum=f4d9942c06bafdb83fb3a825b41bd678' >> options
      echo '_driver_branch=vulkandev' >> options
    fi
# Package type selector
  if [ -z "$_dkms" ]; then
...
DEST_MODULE_LOCATION[3]="/kernel/drivers/video"' dkms.conf
    ...
    for _kernel in "${_kernels[@]}"; do
      ...
      if (( $(vercmp "$_kernel" "7.0") >= 0 )); then
        ...
        [[ "$pkgver" = "595.44.06" || "$pkgver" = "595.44.07" ]] && _whitelist70=( 590* )
        if [[ $pkgver = 470* ]]; then
          cd "$srcdir"/"$_pkg"/kernel-$_kernel
      ...
      if (( $(vercmp "$_kernel" "7.1") >= 0 )); then
        _whitelist70=()
        _whitelist71=( 595* )
        [[ "$pkgver" = "595.44.06" || "$pkgver" = "595.44.07" ]] && _whitelist71=()
      fi
