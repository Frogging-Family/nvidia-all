...
if [ -z "$_driver_version" ] || [ "$_driver_version" = "latest" ] || [ -z "$_driver_branch" ] && [ ! -e options ]; then
  ...
  if [[ -z $CONDITION ]]; then
    ...
    if [ "$CONDITION" = "2" ]; then
      echo '_driver_version=575.51.02' > options
      echo '_md5sum=4fde8f47d366411973b8d62c3860e486' >> options
      echo '_driver_branch=regular' >> options
    elif [ "$CONDITION" = "3" ]; then
      echo '_driver_version=570.133.07' > options
      echo '_md5sum=27c62c49264e1a1da92ebcede9cf76bc' >> options
      echo '_driver_branch=regular' >> options
      ...
      read -p "    Which legacy driver version do you want?`echo $'\n    > 1.560.35.03\n      2.555 series: 555.58.02\n      3.550 series: 550.135\n      4.545 series: 545.29.06\n      5.535 series: 535.183.01\n      6.530 series: 530.41.03\n      7.525 series: 525.147.05\n      8.520 series: 520.56.06\n      9.515 series: 515.86.01\n      10.510 series: 510.85.02\n      11.495 series: 495.46\n      12.465 series: 465.31\n      13.460 series: 460.91.03\n      14.455 series: 455.45.01\n      15.450 series: 450.119.03\n      16.440 series: 440.100 (kernel 5.8 or lower)\n      17.435 series: 435.21  (kernel 5.6 or lower)\n      18.430 series: 430.64  (kernel 5.5 or lower)\n      19.418 series: 418.113 (kernel 5.5 or lower)\n      20.415 series: 415.27  (kernel 5.4 or lower)\n      21.410 series: 410.104 (kernel 5.5 or lower)\n      22.396 series: 396.54  (kernel 5.3 or lower, 5.1 or lower recommended)\n    choice[1-22?]: '`" CONDITION;
      if [ "$CONDITION" = "2" ]; then
    ...
    # This (condition 1) will be treated as the latest Vulkan developer driver.
      ...
      echo '_driver_version=570.123.07' > options
      echo '_md5sum=23d92e224c5efa9a912ad99b9eff4ef1' >> options
      echo '_driver_branch=vulkandev' >> options
    fi
# Package type selector
  if [ -z "$_dkms" ]; then
