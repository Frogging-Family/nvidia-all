...
if [ -z "$_driver_version" ] || [ "$_driver_version" = "latest" ] || [ -z "$_driver_branch" ] && [ ! -e options ]; then
  ...
  if [[ -z $CONDITION ]]; then
    ...
    if [ "$CONDITION" = "2" ]; then
      ...
      echo '_driver_version=525.105.17' > options
      echo '_md5sum=7f656254d842ae6b95277bcb3d33bb88' >> options
      echo '_driver_branch=regular' >> options
    ...
    # This (condition 1) will be treated as the latest Vulkan developer driver.
      ...
      echo '_driver_version=525.47.13' > options
      echo '_md5sum=5b044d7f31637ff5c8f6459f10da727d' >> options
      echo '_driver_branch=vulkandev' >> options
    fi
# Package type selector
  if [ -z "$_dkms" ]; then
